...
export class RenameInstanceDialogComponent {
  ...
  isValid(name: string) {
    return name.trim().length > 0 && !this._siblingInstanceNames.has(name)
  }
  ...
  private _siblingNameValidator(): ValidatorFn {
    return (control: FormControl<string>): ValidationErrors | null => {
      const forbidden = this._siblingInstanceNames.has(control.value)
      return forbidden ? { siblingName: { value: control.value } } : null
    }
  }
